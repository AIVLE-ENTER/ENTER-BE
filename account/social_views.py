...
def google_login(request):
    ...
    token_payload = {
        ...
        'redirect_uri': 'http://localhost:5500/signup_test.html?type=google',
    ...
    if token_response.status_code == 200:
        ...
        user_info_response = requests.get(user_info_url, headers=headers) # 여기까지가 response 받아오는 코드.
        ...
        if user_info_response.status_code == 200:
            ...
            return JsonResponse({"success": False, 'message': 'Failed to fetch user info from Google'}, status=500)
        
    else:
        return JsonResponse({"success": False, "message": "access token을 받아오는데 실패했습니다."}, status=500, json_dumps_params={"ensure_ascii": False},)